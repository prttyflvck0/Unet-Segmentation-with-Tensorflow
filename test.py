...
def save_results(image, mask, pred, save_image_path):
    ...
    pred = grayscale_to_rgb(pred, COLORMAP)
...
def count_obj(pred):
    window_color = (255, 85, 0)

    pred = np.expand_dims(pred, axis=-1)
    pred = grayscale_to_rgb(pred, COLORMAP)

    only_windows = cv2.inRange(pred, window_color, window_color)

    # find contours in the edge map and initialize the total number of
    # shapes found
    cnts = cv2.findContours(only_windows, cv2.RETR_EXTERNAL,
...
if __name__ == "__main__":
    ...
    IMG_W = 416
    NUM_CLASSES = 13
    dataset_path = r'CMP_facade_DB_base/base'
    model_path = os.path.normpath(r'tmp')
    ...
    for x, y in zip(test_x, test_y):
        ...
        print(name, '- windows detected:', count_obj(pred))
